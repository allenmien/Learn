...
{
    public class ConverterHN : IConverter
    {
                ...
                foreach (HtmlNode rowNode in rows)
                {
                    ...
                    if (tdList != null && tdList.Count > 7)
                    {
                        ...
                        if (match != null && match.Success)
                        {
                            ...
                            var responseInfo = this._request.RequestData(new RequestSetting("get", "http://ha.gsxt.gov.cn/business/mortInfoDetail.jspx?id=" + id, "", "0", "dongchandiyaxiangqing"));
                ...
                if (list != null && list.Any())
                {
                    foreach (object obj in list)
                    {
                        ...
                        if (document.Contains("modifyBefore"))
                        {
                            ...
                            {
                                item.before_update = document["modifyBefore"].AsString.Replace("<br/>"," ");
                            }
                        }
                        else
                        {
                        ...
                        if (document.Contains("modifyAfter"))
                        {
                            ...
                            {
                                item.after_update = document["modifyAfter"].AsString.Replace("<br/>", " ");
                            }
                        }
                        else
                        {
                                ...
                                if (match != null && match.Success && !string.IsNullOrEmpty(match.Value))
                                {
                                    ...
                                    if (!string.IsNullOrEmpty(id))
                                    {
                                        ...
                                        elements.Add(new RequestSetting("get",
                                            "http://ha.gsxt.gov.cn" + id, string.Empty, "0", "punishmentdetail"));
