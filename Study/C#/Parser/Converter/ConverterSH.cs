...
{
    public class ConverterSH : IConverter
    {
        ...
        public SummaryEntity ProcessRequestAndParse(RequestInfo requestInfo)
        {
            ...
            if (basicInfo != null)
            {
                ...
                if (table == null)
                {
                    var script = rootNode.SelectSingleNode("//script");
                    if (script != null)
                    {
                        ...
                        if (!string.IsNullOrWhiteSpace(url))
                        {
                            ...
                            basicInfo = inner_responseList.First();
                        }
                        else
                        {
                            Console.WriteLine("basicInfo..." + rootNode.OuterHtml);
                        }
                    }
                }
            ...
            this.LoadAndParseTab01(basicInfo.Data, _enterpriseInfo);
            if (!(requestInfo.Parameters.ContainsKey("platform") && "LIST_API" == requestInfo.Parameters["platform"]))
            {
        ...
        private void LoadAndParseTab01(string responseData, EnterpriseInfo _enterpriseInfo)
        {
            ...
            HtmlNode rootNode = document.DocumentNode;
            Parallel.Invoke(
                ()=>this.LoadBasic(_enterpriseInfo, rootNode),
                () => this.LoadEmployeeAndBranch(_enterpriseInfo, rootNode),
                () => this.LoadCheckup(rootNode),
                () => this.LoadEquityQuality(_enterpriseInfo, rootNode),
                () => this.LoadMortgage(_enterpriseInfo, rootNode),
                () => this.LoadKnowledgeProperty(rootNode, _enterpriseInfo),
            ...
            else
            {
                ...
                if (script != null)
                {
                    ...
                    if (!string.IsNullOrWhiteSpace(url))
                    {
                        ...
                        var responseList = _request.GetResponseInfo(requests);
                        if (responseList != null && responseList.Any())
                        {
                            ...
                            this.LoadLicenseInfo(_enterpriseInfo, inner_rootNode);
                        }
                    }
                    else
                    {
                        Console.WriteLine("LoadLicenseInfo..." + rootNode.OuterHtml);
                    }
                }
            }
                ...
                if (script != null)
                {
                    ...
                    if (!string.IsNullOrWhiteSpace(url))
                    {
                        ...
                        if (responseList != null && responseList.Any())
                        {
                            ...
                            inner_document.LoadHtml(responseList.First().Data);
                            var inner_rootNode = inner_document.DocumentNode;
                            this.LoadPunishments(_enterpriseInfo, inner_rootNode);
                        }
                    }
                    else
                    {
                        Console.WriteLine("LoadPunishments..." + rootNode.OuterHtml);
                    }
                }
            }
                ...
                if (script != null)
                {
                    ...
                    if (!string.IsNullOrWhiteSpace(url))
                    {
                        ...
                        if (responseList != null && responseList.Any())
                        {
                            ...
                            inner_document.LoadHtml(responseList.First().Data);
                            var inner_rootNode = inner_document.DocumentNode;
                            this.LoadAbnormals(inner_rootNode);
                        }
                    }
                    else
                    {
                        Console.WriteLine("LoadAbnormals..." + rootNode.OuterHtml);
                    }
                }
            }
            else
        ...
        private void LoadEmployeeAndBranch(EnterpriseInfo _enterpriseInfo, HtmlNode rootNode)
        {
            ...
            div = rootNode.SelectSingleNode("//div[@id='layout-01_02_02']");
            if (div != null)
            {
            ...
            if (tdList != null && tdList.Count > 3)
            {
                ...
                try
                {
                    if (!string.IsNullOrWhiteSpace(uuid))
                    {
                        ...
                        {
                            Method = "get",
                            Url = string.Format("http://sh.gsxt.gov.cn/notice/notice/view_investor?uuid={0}", uuid),
                            IsArray = "0",
            ...
            if (tdList != null && tdList.Count > 3)
            {
                ...
                try
                {
                    if (!string.IsNullOrWhiteSpace(uuid))
                    {
                        ...
                        {
                            Method = "get",
                            Url = string.Format("http://sh.gsxt.gov.cn/notice/notice/view_initiator?uuid={0}", uuid),
                            IsArray = "0",
                            Name = "investor_detials",
                            Data = ""
                        });
            ...
            if (tables == null)
            {
                ...
                if (script != null)
                {
                    ...
                    if (!string.IsNullOrWhiteSpace(url))
                    {
                        ...
                        if (responseList != null && responseList.Any())
                        {
                            this.LoadAndParseInvestorDetails(partner, responseList.First().Data);
                        }
                    }
                    else
                    {
                        Console.WriteLine(rootNode.OuterHtml + "【LoadAndParseInvestorDetails】");
                    }
                }
            }
            else
            ...
            else
            {
                ...
                if (script != null)
                {
                    ...
                    if (!string.IsNullOrWhiteSpace(url))
                    {
                        ...
                        if (responseList != null && responseList.Any())
                        {
                            this.LoadAndParseMortgageDetails(mortgage, responseList.First().Data);
                        }
                    }
                    else
                    {
                        Console.WriteLine(rootNode.OuterHtml + "【LoadAndParseMortgageDetails】");
                    }
                }
            }
        }
        ...
        private void LoadAndParseReport(string responseData, EnterpriseInfo _enterpriseInfo)
        {
            ...
            if (tables == null) 
            {
                tables = rootNode.SelectNodes("//div[@rel='layout-02_01_01']/div/table");
            }
            ...
            else
            {
                ...
                if (script != null)
                {
                    ...
                    if (!string.IsNullOrWhiteSpace(url))
                    {
                        ...
                        if (responseList != null && responseList.Any())
                        {
                            this.LoadAndParseReport(responseList.First().Data, _enterpriseInfo);
                        }
                    }
                    else
                    {
                        Console.WriteLine("LoadAndParseReport..." + rootNode.OuterHtml);
                    }
                }
            }
        }
