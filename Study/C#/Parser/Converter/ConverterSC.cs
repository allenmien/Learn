...
{
    public class ConverterSC : IConverter
    {
                        ...
                        if (trList != null && trList.Any())
                        {
                            ...
                            foreach (HtmlNode rowNode in trList)
                            {
                                ...
                                if (tddList != null && tddList.Count > 4)
                                {
                                    ...
                                    changeRecord.before_content = HideIdentityNo(tddList[2].InnerText);
                                    changeRecord.after_content =HideIdentityNo( tddList[3].InnerText);
            ...
            if (tdList != null && tdList.Count > 3)
            {
                ...
                try
                {
                    if (!string.IsNullOrWhiteSpace(uuid))
                    {
                        ...
                        {
                            Method = "get",
                            Url = string.Format("http://sc.gsxt.gov.cn/notice/notice/view_investor?uuid={0}", uuid),
                            IsArray = "0",
            ...
            if (tdList != null && tdList.Count > 3)
            {
                ...
                try
                {
                    if (!string.IsNullOrWhiteSpace(uuid))
                    {
                        ...
                        {
                            Method = "get",
                            Url = string.Format("http://sc.gsxt.gov.cn/notice/notice/view_initiator?uuid={0}", uuid),
                            IsArray = "0",
                            Name = "investor_detials",
                            Data = ""
                        });
        ...
        private string HideIdentityNo(string content)
        {
            return Regex.Replace(content,"(\\d{17}(\\d|X|x)|(\\d{18})|(\\d{15}))","******");
        }
    }
}