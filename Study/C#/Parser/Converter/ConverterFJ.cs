...
{
    public class ConverterFJ : IConverter
    {
        ...
        public SummaryEntity ProcessRequestAndParse(RequestInfo requestInfo)
        {
            ...
            if (!(requestInfo.Parameters.ContainsKey("platform") && "LIST_API" == requestInfo.Parameters["platform"]))
            {
                ...
                Parallel.ForEach(responseList, new ParallelOptions { MaxDegreeOfParallelism = _parallelCount }, item => this.ParseResponse(item));
            }
            else
            {
            ...
            if (tdList != null && tdList.Count > 3)
            {
                ...
                try
                {
                    if (!string.IsNullOrWhiteSpace(uuid))
                    {
                        ...
                        {
                            Method = "get",
                            Url = string.Format("http://fj.gsxt.gov.cn/notice/notice/view_investor?uuid={0}", uuid),
                            IsArray = "0",
            ...
            if (tdList != null && tdList.Count > 3)
            {
                ...
                try
                {
                    if (!string.IsNullOrWhiteSpace(uuid))
                    {
                        ...
                        {
                            Method = "get",
                            Url = string.Format("http://fj.gsxt.gov.cn/notice/notice/view_initiator?uuid={0}", uuid),
                            IsArray = "0",
                            Name = "investor_detials",
                            Data = ""
                        });
