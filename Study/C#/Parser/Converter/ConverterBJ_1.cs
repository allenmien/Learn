...
{
    public class ConverterBJ : IConverter
    {
        ...
        string _http = "http://bj.gsxt.gov.cn";
        ...
        public SummaryEntity ProcessRequestAndParse(RequestInfo requestInfo)
        {
            ...
            {
                this._requestXml = new RequestXml(requestInfo.CurrentPath, "BJ" + "_API");
            }
            else
            {
                this._requestXml = new RequestXml(requestInfo.CurrentPath, "BJ");
            }
            ...
            if (urls != null)
            {
                ...
                foreach (var url in urls)
                {
                    ...
                    else if (url.StartsWith(http + "/gjjbj/gjjQueryCreditAction!zyryFrame.dhtml"))//主要人员
                    {
                        ...
                        request.Url = string.Format("http://bj.gsxt.gov.cn/gjjbj/gjjQueryCreditAction!zyryFrame.dhtml?flag=more&ent_id={0}&credit_ticket={1}",
                    ...
                    else if (url.StartsWith(http + "/gjjbj/gjjQueryCreditAction!fzjgFrame.dhtml"))//分支机构
                    {
                        ...
                        request.Url = string.Format("http://bj.gsxt.gov.cn/gjjbj/gjjQueryCreditAction!fzjgFrame.dhtml?flag=more&ent_id={0}&regno={1}&credit_ticket={2}",
                            _requestInfo.Parameters["entId"], _regno, _requestInfo.Parameters["credit_ticket"]);
                        requests.Add(request);
                    }
                    else if (url.StartsWith(http + "/gjjbj/gjjQueryCreditAction!biangengFrame.dhtml"))//变更信息
                    {
                    ...
                    else if (url.StartsWith(http + "/gsgs/gsxzcfAction!list_ccjcxx.dhtml"))//抽查检查
                    {
        ...
        void LoadAndParsePartnerInfo(HtmlNode rootNode)
        {
            ...
            if (table != null)
            {
                ...
                Parallel.ForEach(arr, item => this.LoadAndParsePartnerInfo_Parallel(item, form.Attributes["action"].Value));
                int index = 1;
                if (_enterpriseInfo.partners.Any())
                {
                    foreach (var item in _enterpriseInfo.partners)
                    {
                        item.seq_no = index;
                        index++;
                    }
        ...
        #region LoadAndParsePartnerInfo_Parallel
        void LoadAndParsePartnerInfo_Parallel(int i, string partnerInfobypage_url)
        {
            var request = this.CreateRequest();
            request.AddOrUpdateRequestParameter("partnerInfobypage_url", partnerInfobypage_url);
        ...
        void LoadAndParsePartnerInfoByPage(HtmlNode table)
        {
            if (table != null)
            {
                ...
                {
                    trs.Remove(0);
                    Parallel.ForEach(trs, tr => this.LoadAndParsePartnerInfoByPage_Parallel(tr));
                }
            }
        }
        #endregion

        #region LoadAndParsePartnerInfoByPage_Parallel
        void LoadAndParsePartnerInfoByPage_Parallel(HtmlNode tr)
        {
        ...
        void LoadAndParseChangeRecordInfo(HtmlNode rootNode)
        {
            ...
            if (table != null)
            {
                ...
                Parallel.ForEach(arr, item => this.LoadAndParseChangeRecordInfo_Parallel(item));
                int index = 1;
                if (_enterpriseInfo.changerecords != null && _enterpriseInfo.changerecords.Any())
                {
                    foreach (var item in _enterpriseInfo.changerecords)
                    {
                        item.seq_no = index;
                        index++;
                    }
        ...
        #region LoadAndParseChangeRecordInfo_Parallel
        void LoadAndParseChangeRecordInfo_Parallel(int i)
        {
            var request = this.CreateRequest();
            request.AddOrUpdateRequestParameter("pageNos", i.ToString());
            request.AddOrUpdateRequestParameter("pageNo", (i - 1).ToString());
        ...
        void LoadAndParseChangeRecordInfoByPage(HtmlNode table)
        {
            ...
            if (table != null)
            {
                ...
                if (trs != null && trs.Any())
                {
                    ...
                    trs.Remove(0);
                    Parallel.ForEach(trs, tr => this.LoadAndParseChangeRecordInfoByPage_Parallel(tr));
                }
            }
        }
        #endregion

        #region LoadAndParseChangeRecordInfoByPage_Parallel
        void LoadAndParseChangeRecordInfoByPage_Parallel(HtmlNode tr)
        {
            var request = this.CreateRequest();
            var tds = tr.SelectNodes("./td");
        ...
        void LoadAndParseReport_Parallel(HtmlNode tr)
        {
            ...
            if (tds != null && tds.Any() && tds.Count == 4)
            {
                ...
                if (reportsNeedToLoad.Contains(report.report_year) || !reportsNeedToLoad.Any())
                {
                    var a = tds.Last().SelectSingleNode("./a");
                    if (a != null)
                    {
